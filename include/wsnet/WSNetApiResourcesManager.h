...
enum class ApiResourcesManagerNotification {
    kLoginOk = 0,     // login/signup was successful and API resources (SessionStatus, Locations, ServerCredentials(both openvpn and ikev2), ServerConfigs, PortMap, StatisIPS) are ready for use
    kLoginFailed,     // login/signup failed, returns error code in LoginResult variable and error message in string
...
public:
    ...
    // authTokenSignup call, must be called before signup API call. Required for two stage signup + CAPTCHA
    virtual void authTokenSignup(const std::string &username, bool useAsciiCaptcha) = 0;

    // signup with username/password and other optional parameters
    // optionally send captcha data
    // Returns the result in the notifications kSignupOk/kSignupFailed
    virtual void signup(const std::string &username, const std::string &password, const std::string &referringUsername,
    ...
    virtual std::string authTokenResult() const = 0;
    virtual std::string amneziawgUnblockParams() const = 0;
    virtual std::string amneziawgConfigId() const = 0;
